...
import { Check, ImageUp, Lock, Pencil, TriangleAlert } from "lucide-solid";
...
import { pookalamState, QUERY_KEYS, shell } from "~/lib/queries";
...
export function PookalamSubmitView() {
  ...
  createEffect(() => {
    if (editingInitialized || !state()) return;
    editingInitialized = true;
    setEditing(!state()!.mine);
  });
  ...
  const onSubmit = async (event: SubmitEvent) => {
    ...
    try {
      ...
      setImageDataUrl("");
      setEditing(false);
      await revalidate(QUERY_KEYS.pookalamState);
      prefilled = false;
  ...
  return (
    <main class="container space-y-8 py-6">
      ...
      <Show when={state()} fallback={<LoadingScreen compact message="Loading submission state…" />}>
        ...
        >
          {/* Success state — shown after submission, collapsed by default. */}
          <Show when={state()!.mine && !editing()}>
            <div class="card pop-teal space-y-3">
              <div class="flex items-center gap-2">
                <span
                  class="grid h-8 w-8 place-items-center rounded-full bg-white"
                  style={{ border: "var(--ink-w) solid var(--ink)" }}
                >
                  <Check size={18} strokeWidth={3} />
                </span>
                <h2 class="text-lg font-black m-0">You have submitted successfully</h2>
              </div>
              <p class="text-sm font-semibold m-0">Your pookalam is in.</p>
            ...
            <Show when={editing() || !state()!.mine}>
              ...
              <form class="card card-plain space-y-4" onSubmit={onSubmit}>
                <label class="block space-y-1">
                  ...
                  />
                  <span class="comment">
                    GitHub, a Gist, GitLab, Codeberg, CodePen or similar. Must include an
                    Open-Source License (MIT, Apache 2.0, GPL, etc.) and run instructions. Nobody
                    sees this until results are out.
                  </span>
                </label>
