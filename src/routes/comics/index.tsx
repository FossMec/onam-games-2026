...
import { BookOpen, ChevronLeft, ChevronRight, Layers, Send, Sparkles } from "lucide-solid";
...
export default function ComicsPage() {
  ...
  const [flipping, setFlipping] = createSignal<"next" | "prev" | null>(null);
  ...
  const goToIssue = (nextIssue: number, dir: "next" | "prev") => {
    if (flipping() || nextIssue === currentIssue()) return;
    setFlipping(dir);
    setTimeout(() => {
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .flip-right-leaf {
          animation: flipRightToLeft 0.5s cubic-bezier(0.4, 0, 0.2, 1) forwards;
          transform-origin: left center;
          transform-style: preserve-3d;
          z-index: 30;
        }
        .flip-left-leaf {
          animation: flipLeftToRight 0.5s cubic-bezier(0.4, 0, 0.2, 1) forwards;
      ...
      <Show when={viewMode() === "flip"}>
        <div class="h-[calc(100dvh-4.25rem)] w-full max-w-5xl mx-auto px-2 sm:px-4 py-2 flex flex-col justify-between overflow-hidden select-none">
          {/* Top Header */}
          <header class="flex items-center justify-between gap-2 shrink-0 py-1 border-b-2 border-[var(--ink)]/15">
            <div class="flex items-center gap-2">
              <span
                class="badge text-[11px] sm:text-xs font-black px-2 py-0.5"
                style={{
              ...
              <span class="font-black text-xs sm:text-sm text-[var(--ink)]">
          ...
          <div class="book-stage flex-1 min-h-0 flex items-center justify-center p-1 sm:p-2 overflow-hidden">
            ...
            >
              ...
              <div
                class={`w-1/2 h-full flex flex-col justify-center overflow-hidden relative p-0 ${
                  flipping() === "prev" ? "flip-left-leaf" : ""
                }`}
                style={{
                  background: "var(--paper)",
                }}
